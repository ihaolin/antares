...
public class AppServiceImpl implements AppService {
     ...
     */
    private final LoadingCache<String, App> APP_NAME_CACHE = CacheBuilder.newBuilder()
            .expireAfterAccess(5, TimeUnit.MINUTES).build(new CacheLoader<String, App>() {
                @Override
                public App load(@Nonnull String appName) {
                    return appDao.findByName(appName);
                }
            });

    private final LoadingCache<Long, App> APP_ID_CACHE = CacheBuilder.newBuilder()
            .expireAfterAccess(5, TimeUnit.MINUTES).build(new CacheLoader<Long, App>() {
                @Override
                public App load(@Nonnull Long appId) {
